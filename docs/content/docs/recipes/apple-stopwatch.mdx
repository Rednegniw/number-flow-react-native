...
description: iOS-style stopwatch with centisecond precision
full: true
---

An Apple Stopwatch replica with centisecond-precision display and circular start/stop/reset buttons, composing three `NumberFlow` instances for the `MM:SS.CC` format.
...
const PAD_TWO = { minimumIntegerDigits: 2 };
...
export default function AppleStopwatch() {
  ...
  return (
    <View style={{
      ...
      {/* Time display — three NumberFlows: MM: SS. CC */}
...
<Callout type="info">
This recipe composes three `NumberFlow` instances side by side — minutes with `suffix=":"`, seconds with `suffix="."`, and centiseconds with `animated={false}`. The `fontVariant: ['tabular-nums']` style prevents layout jitter from varying digit widths, and `digits={{ 1: { max: 5 } }}` on seconds constrains the tens digit to 0–5 for correct wrapping at the 59→00 boundary.
</Callout>
