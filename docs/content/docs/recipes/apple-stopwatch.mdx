...
<Callout type="info">
The `centiseconds` prop on `TimeFlow` renders a `.CC` segment after seconds using fixed keys (`c10`, `c1`) for stable animations. Combined with `fontVariant: ['tabular-nums']`, this creates a jitter-free stopwatch display with a single component — no need to compose multiple `NumberFlow` instances.
</Callout>
