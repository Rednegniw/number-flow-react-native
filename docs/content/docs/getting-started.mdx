...
```tsx
// All components
import { NumberFlow, TimeFlow, SkiaNumberFlow, SkiaTimeFlow } from 'number-flow-react-native';

// Native only (smaller bundle if not using Skia)
...
<Callout type="info">
If you are not using Skia, import from `number-flow-react-native/native` to avoid pulling in the optional Skia dependency.
</Callout>
