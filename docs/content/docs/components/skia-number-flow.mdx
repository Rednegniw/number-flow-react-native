...
```SnackPlayer name=SkiaNumberFlow%20Basic dependencies=number-flow-react-native@0.1.3,react-native-reanimated,@shopify/react-native-skia
...
function ScrubSlider() {
  const font = useSkiaFont(require('./assets/Inter-Medium.otf'), 32);
