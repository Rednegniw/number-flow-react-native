...
- **Native** — `MaskedView` wraps the content. The mask element is a column of `View` strips with graduated opacity (`rgba(0,0,0,alpha)`) forming a gradient from transparent to opaque.
