...
export default function TrendControl() {
  ...
  return (
    <ExampleLayout>
      ...
      {/* Trend controls */}
      <View style={{ gap: 8 }}>
        <Text style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', textAlign: 'center' }}>Trend Direction</Text>
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <Button title="Auto" onPress={() => setTrend(undefined)} />
          <Button title="Up (1)" onPress={() => setTrend(1)} />
          <Button title="Down (-1)" onPress={() => setTrend(-1)} />
          <Button title="Shortest (0)" onPress={() => setTrend(0)} />
        </View>
      </View>
    </ExampleLayout>
  );
